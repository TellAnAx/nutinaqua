...
- reliable comparison of nutrient inclusion is possible for N and P data only, considering the number of observations in the dataset.
- N: no significant difference between any of the feeds
...
read_rds(here("output", "interm", "feed_cleaned.rds")) %>% 
  mutate(
    ...
    category = fct_relevel(cat2, "old", "commercial", "aquaponic", "experimental")
  ) %>% 
  ggplot(aes(x = substance, y = value, fill = category)) + 
  geom_boxplot() +
  facet_wrap(facets = vars(substance), scales = "free") + 
  scale_fill_manual(values = c("grey", "lightblue", "lightgreen", "gold")) +
  theme_bw() +
  theme(axis.ticks.x = element_blank(),
        axis.text.x = element_blank()) +
  labs(x = "",
       y = "Inclusion rate (mmol/kg)",
       fill = "class")
