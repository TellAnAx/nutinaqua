...
```{r}
dailyFreshwater <- c(50, 100, 1000) # L
```
...
```{r}
tables$water_combined_quantiles_tap <- read_rds(here("output", "interm", "permutation_combined.rds")) %>% 
...
```{r}
tables$water_combined_quantiles_rain <- read_rds(here("output", "interm", "permutation_combined.rds")) %>% 
  filter(source != "aquafeed" & dailyFreshwater != "1000") %>% 
  group_by(substance, source2, dailyFreshwater) %>% 
  summarise(
    median = median(percentage),
