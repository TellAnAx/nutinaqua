...
read_rds(here("output" , "interm", "water_cleaned_tap_before_MLE.rds")) %>% 
  ...
  write_rds(here("output", "interm", "water_cleaned_tap_MLE.rds")) %>% print() 
```
...
read_rds(here("output", "interm", "water_cleaned_tap_MLE.rds")) %>% select(-c(n, min, conf.025, sd, cv, conf.975, max)) %>% 
...
data$permutation_water_tap_with_assumptions <- map_dfr(dailyFreshwater, ~{
  read_rds(here("output", "interm", "water_cleaned_tap.rds")) %>% 
