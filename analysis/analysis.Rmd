...
The amount of daily feed fed is normalised to 1 kg of feed instead of going for the initial assumptions. This makes it better comparable.
...
```{r}
data$permutation_with_assumptions_tap <- data$permutation_feed_with_assumptions %>% 
  left_join(data$permutation_water_tap_with_assumptions, 
            relationship = "many-to-many") %>% 
  ...
  mutate(source = fct_recode(source, aquafeed = "perc_feed", `tap water` = "perc_water"),
         substance = fct_relevel(substance, "N", "P", "K", 
                              "Ca", "Mg", "S", 
                              "Fe", "Cu", "Zn")) %>%
  write_rds(here("output", "interm", "permutation.rds"))
```

```{r}
read_rds(here("output", "interm", "permutation.rds")) %>% 
...
```{r}
five_list <- read_rds(here("output", "interm", "permutation.rds")) %>% 
  filter(source != "aquafeed") %>% 
  group_by(substance, dailyFreshwater) %>% 
  summarise(quantile = quantile(percentage, .05))

median_list <- read_rds(here("output", "interm", "permutation.rds")) %>% 
  filter(source != "aquafeed") %>% 
  group_by(substance, dailyFreshwater) %>% 
  summarise(median = median(percentage))

ninetyfive_list <- read_rds(here("output", "interm", "permutation.rds")) %>% 
  filter(source != "aquafeed") %>% 
  group_by(substance, dailyFreshwater) %>% 
