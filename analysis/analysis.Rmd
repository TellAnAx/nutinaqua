...
read_rds(here("output", "interm", "water_cleaned_tap_MLE.rds")) %>% select(-c(n, min, conf.025, sd, cv, conf.975, max)) %>% 
  ...
  #group_by(analyte, belowLimit) %>% count()
  #distinct(conc2) %>% 
  #group_by(analyte) %>% count()
...
In the case of the **nitrogenous compounds**, \ce{NH4+} and \ce{NO2-} were removed from the dataset and \gls{TN} was assumed to be solely dependent on \ce{NO3-} as it would have the highest contribution of the mentioned compounds to the TN. 

```{r}
read_rds(here("output", "interm", "water_cleaned_tap_before_MLE.rds")) %>% 
  filter(analyte == "NO3" & belowLimit == FALSE)
```
