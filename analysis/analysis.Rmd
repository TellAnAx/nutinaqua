...
- **only water analyses from Europe were considered**
- **water analysis reports were matched with the closest bigger city and only one observation per bigger city was included into the dataset**
...
results$n_feeds_commercial <- data$feed %>% filter(cat1 == "commercial" & ref_id != "Tacon1983") %>% nrow()

results$n_feeds_commercial_datasheet <- data$feed %>% filter(cat1 == "commercial" & ref_id == "datasheet") %>% nrow()
...
- `r results$n_feeds_commercial` commercial feeds 
- `r results$n_feeds_commercial_datasheet` commercial feeds with data from datasheets
- `r results$n_feeds_commercial_literature` commercial feeds with data from literature
- `r results$n_feeds_experimental` experimental feeds
...
- `r results$n_observations_water_tap` tap water analysis reports
...
data$water_cleaned_tap <- read_rds(here("output", "interm", "water_cleaned.rds")) %>% 
  ...
  mutate(
    NO3_mgL = if_else(is.na(NO3_mgL),0,NO3_mgL*0.23) + if_else(is.na(NO2_mgL),0,NO2_mgL*0.3) + if_else(is.na(NH4_mgL),0,NH4_mgL*0.7), # convert to elemental N (TN_dissolved)
  ...
  write_rds(here("output", "interm", "water_cleaned_tap_before_MLE.rds"))
```  

```{r}
read_rds(here("output" , "interm", "water_cleaned_tap_before_MLE.rds")) %>% 
  # summarise data
  ...
  summarise(
    ...
    mutate(analyte = if_else(analyte == "NO3", "N", analyte), 
         analyte = if_else(analyte == "SO4", "S", analyte),
         analyte = if_else(analyte == "PO4", "P", analyte)) %>% 
  filter(!analyte %in% c("NH4", "NO2")) %>% 
  write_rds(here("output", "interm", "water_cleaned_tap_MLE.rds"))
```

```{r create_table_water_tap}
tables$water_tap <- read_rds(here("output", "interm", "water_cleaned_tap_MLE.rds")) %>% 
  select(analyte, unit, n, min, meanconf, sd, cv, max) %>% 
  table_styler() %>% 
  rename(mean = "meanconf") %>% 
  write_csv(here("output", "tables", "water_tap_censored.csv"))
```
...
In the next step, the mean MLE estimate is used as replacement for those values that were found to be below the detection limit.
*Would it be better if only one of the means would remain in the dataset? Otherwise, those values would have a high weight in the resulting dataset.*

```{r create_rds_water_cleaned_tap}
read_rds(here("output", "interm", "water_cleaned_tap_MLE.rds")) %>%
  select(-c(n, min, sd, cv, max)) %>% 
  select(-c(conf.025, conf.975)) %>% 
  right_join(read_rds(here("output", "interm", "water_cleaned_tap_before_MLE.rds"))) %>% 
  mutate(conc2 = if_else(belowLimit == TRUE, meanconf, conc)) %>% 
...
```{r plot_percentage_not_detectable_tap, include=TRUE}
read_rds(here("output", "interm", "water_cleaned_tap_before_MLE.rds")) %>% 
  group_by(analyte) %>% 
  summarise(total = n(),
            not_detectable = sum(belowLimit),
            perc_not_detectable = not_detectable/total*100) %>% 
  ggplot(aes(x = fct_rev(analyte), 
             y = perc_not_detectable)) + 
  geom_col() +
  ...
  theme_bw() + 
  labs(x = "",
       y = "Percentage not detectable")

ggsave(here("output", "plots", "not_detectable_tap.png"),
...
results$n_studies_water_rain <- read_rds(here("output", "interm", "water_cleaned.rds")) %>% 
  filter(source == "rain") %>% 
  distinct(ref_id) %>% nrow()
```

- `r results$n_observations_water_rain` rain water analyses from literature
- `r results$n_studies_water_rain` studies from which the data was derived
...
data$water_cleaned_rain <- read_rds(here("output", "interm", "water_cleaned.rds")) %>% 
  ...
  rowwise() %>% 
    # convert concentrations
  mutate(
    N_mgL = if_else(is.na(NO3_mgL),0,NO3_mgL*0.23) + if_else(is.na(NO2_mgL),0,NO2_mgL*0.3) + if_else(is.na(NH4_mgL),0,NH4_mgL*0.7), # convert to elemental N (TN_dissolved)
...
```{r}
#
results$n_observations_water_rain <- data$water_cleaned %>% 
  filter(source == "rain") %>% nrow()
...
```{r}
data$water_combined_tap <- read_rds(here("output", "interm", "water_cleaned_tap_MLE.rds")) %>% 
...
data$permutation_feed_digestibility <- tibble(
  N = seq(0.85, 0.95, 0.05), # Roy et al., 2022
  P = seq(0.35, 0.45, 0.05), # Sugiura et al., 2018
  K = seq(0.45, 0.55, 0.05), # Roy et al., 2022
  Ca = seq(0.45, 0.55, 0.05), # Roy et al., 2022
  Mg = seq(0.45, 0.55, 0.05), # Roy et al., 2022
  S = seq(0.45, 0.55, 0.05), # Roy et al., 2022
  B = seq(0.45, 0.55, 0.05), # Roy et al., 2022
  Fe = seq(0.45, 0.55, 0.05), # Roy et al., 2022
  Mn = seq(0.45, 0.55, 0.05), # Roy et al., 2022
  Cu = seq(0.45, 0.55, 0.05), # Roy et al., 2022
  Zn = seq(0.45, 0.55, 0.05) # Roy et al., 2022
) %>% 
...
```{r create_data_combinations_with_retention_tap}
...
```{r create_data_combinations_with_retention_rain}
...
data$permutation_with_assumptions_combined %>%   
  ...
  mutate(
    ...
    write_rds(here("output", "interm", "permutation_combined_percentage_wide.rds"))
```

```{r}
read_rds(here("output", "interm", "permutation_combined_percentage_wide.rds")) %>% 
  pivot_longer(c(perc_feed, perc_water),
               names_to = "source",
               values_to = "percentage") %>% 
...
read_rds(here("output", "interm", "permutation_combined_percentage.rds")) %>% 
  ...
  mutate(dailyFreshwater = as.character(dailyFreshwater),
         dailyFreshwater = fct_recode(dailyFreshwater,
                                      ...
                                      `1000 L/kg` = "1000"),
         substance = fct_relevel(substance, "N", "P", "K", 
                                 "Ca", "Mg", "S", "Fe", "Cu", "Zn")) %>% write_rds(here("output", "interm", "permutation_combined_percentage_shiny.rds")) %>% 
...
alk_stats <- read_rds(here("output", "interm", "permutation_combined_absolute.rds")) %>% 
  ...
  group_by(substance, dailyFreshwater) %>% 
  summarise(mean = mean(value)) %>% 
  mutate(source = "alkalinity") %>% 
  bind_rows(stuff_stats) %>%
...
```{r eval=FALSE}
stuff_stats <- read_rds(here("output", "interm", "permutation_combined_absolute.rds")) %>% 
  filter(source2 != "rain" & dailyFreshwater == 100) %>% 
  pivot_longer(c("water", "feed"), names_to = "source", values_to = "value") %>% 
  group_by(source, substance) %>% 
  summarise(
  mean = mean(value),
  sd = sd(value)
  ) %>% 
...
read_rds(here("output", "interm", "permutation_combined_absolute.rds")) %>% 
  filter(substance == "N") %>% 
  distinct(feed, source2, water, dailyFreshwater) %>% 
  ungroup() %>% 
  select(-substance) %>% 
  ...
  mutate(source = "alkalinity") %>% 
  bind_rows(stuff_stats) %>% arrange(substance) %>% 
  filter(substance %in% c("N", "P", "K", "Ca", "Mg", "S", "Fe", "Cu", "Zn")) %>%

  pivot_wider(names_from = "source", values_from = c("mean", "sd"), values_fill = 0) %>%
  mutate(
    sum_mean = mean_alkalinity + mean_feed + mean_water,
    sum_sd = sd_alkalinity + sd_feed + sd_water,
    perc_mean_alkalinity = mean_alkalinity/sum_mean * 100,
    perc_mean_feed = mean_feed/sum_mean * 100,
    perc_mean_water = mean_water/sum_mean * 100,
    
    perc_sd_alkalinity = sd_alkalinity/sum_mean * 100,
    perc_sd_feed = sd_feed/sum_mean * 100,
    perc_sd_water = sd_water/sum_mean * 100
  ) %>% 
  pivot_longer(cols = perc_mean_alkalinity:last_col(),
               names_to = c(".value", "source"),
               names_pattern = "(mean_|sd_)(.*)",
               values_to = c("mean", "sd")) %>% 
  select(substance, source, mean_, sd_) %>% 
  rename(mean = "mean_", sd = "sd_") %>%
  mutate(
    substance = fct_relevel(substance, "N", "P", "K", "Ca", "Mg", "S", "Fe", "Cu", "Zn"),
    source = fct_relevel(source, "feed", "water", "alkalinity")) %>%
  
  ggplot(aes(x = mean, y = fct_rev(substance), fill = fct_rev(source))) + 
  geom_col(position = position_dodge()) +
  geom_errorbar(aes(y = substance, 
                    xmin = if_else(mean - sd <= 0, 0.001, mean - sd),
                    xmax = if_else(mean + sd >= 100, 99.999, mean + sd)),
                    position = position_dodge(width = 0.9), 
                width = 0.5, alpha = 0.7) + 
  scale_fill_manual(values = c("#707071", "#ADD3DD", "#8A4F21")) +
  labs(y = "",
       x = "% contribution") +
  lims(x = c(0,100)) +
  theme(legend.position = "none",
        axis.title.x = element_blank(),
        axis.text.y = element_text(face = "bold", family = "Roboto", colour = "black"),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        panel.grid = element_blank(),
        panel.background = element_blank())

ggsave(here("output", "plots", "graphical_abstract.png"), 
