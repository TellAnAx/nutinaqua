...
data$feed_cleaned <- data$feed[[1]] %>% 
  ...
  write_rds(here("interm", "feed_stattest.rds")) %>% 
  create_plot_data() %>% 
  drop_na(value) %>% 
...
read_rds(here("interm", "feed_stattest.rds")) %>% 
  tukey_hsd(P_gkg ~ cat2) 
```
...
Both fish and plants require carbon and oxygen in large quantities. Further essential macronutrients for both are calcium, phosphorus, magnesium, iron, copper, manganese, and zinc. These elements are required for physiological functions such as the development and maintenance of the skeletal system, processes such as signal transmission, osmoregulation, as reactive centers of metalloenzymes (e.g., Haemoglobin, Chlorophyll), and others [@Lall2021; @Marschner2012]. 

- differences in requirements of the nutrients which are needed by both fish and plants

Nutrients relevant only for plants are 
...
%\begin{figure}
%\centering
%  \includegraphics{../plots/substance_flows.png}
