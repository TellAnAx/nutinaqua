...
Ammonium is excreted by the fish as a metabolic end product. Per mole of converted ammonium, two moles of hydronium ions are released. This results in a steady drop of the pH towards areas in which the metabolic activity of the nitrifiers is inhibited [@Ward2011]. This development must therefore be counteracted. For this purpose, a range of alkalinity supplements is used (see Table \ref{tab:alkalinity}). 

```{=tex}
\input{../latex/table_alkalinitysuppl.tex}
```

It can be derived from equation \ref{eq:nitrification} that the amount of hydronium ions to be neutralised is proportional to the amount of feed. As a rule of thumb, approximately 


Alkalinity supplements play a special role with regards to nutrient inputs. 

- very high input of the cation, which is present in much lower amounts in the feed and the water
- Variability negligible because pure substance is purchased
