...
Excluding speciality feeds, the range in which phosphorus can be found in commercial fish feeds is comparably narrow, ranging from \SIrange{7}{18}{\gpkg}. \SI{11(2)}{\gpkg} is found in the feeds on average with no statistical differences among species.
...
data$feed_commercial[[1]] %>% 
  create_plot_data() %>% 
  filter(reference_id == "datasheet" & 
           !feed_category4 %in% c("aquaponic", "pond", "age")) %>%
  select(c(producer:feed_category4, name:value)) %>% 
  drop_na(value) %>% 
...
```{r plot_commercial_datasheet, include=TRUE}
data$feed_commercial[[1]] %>% 
  create_plot_data() %>% 
  filter(reference_id == "datasheet" & 
           !feed_category4 %in% c("aquaponic", "pond", "age")) %>%
...
```{r plot_commercial_studies, include=TRUE}
data$feed_commercial[[1]] %>% 
  create_plot_data() %>% 
  filter(reference_id != "datasheet") %>% 
  select(c(producer:feed_category4, name:value)) %>% 
...
- some commercial feeds were used in aquaponics studies. There is thus more nutrient composition data available.
...
```{r plot_experimental_studies, include=TRUE}
