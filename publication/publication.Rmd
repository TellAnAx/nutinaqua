...
The ASTAF-PRO system [@Kloas2015] was chosen as a model system for the estimation of aquafeed, alkalinity supplement, and source water mass flows into an exemplary aquaponic system. The sizing of the chosen system is comparable with results from a recent survey targeting aquaponics production sites [@Pattillo2020] and its system components are well-described. All model data are summarized in table \ref{tab:assumptions}.
...
These elements are required for physiological functions such as the development and maintenance of the skeletal system, processes such as signal transmission, osmoregulation, as reactive centers of metalloenzymes (e.g., Haemoglobin, Chlorophyll), and others [@Lall2021; @Marschner]. 
...
After exclusion of specialty feeds, the range in which phosphorus, potassium, and calcium can be found in modern commercial fish feeds is comparably narrow, ranging from \SIrange{7}{18}{\gpkg}, \SIrange{10}{13}{\gpkg}, and \SIrange{12}{20}{\gpkg}, respectively. These nutrients also show the lowest \gls{cv} (between \SIrange{20}{21}{\p}). The average inclusion rates did not significantly change in comparison with past values, being \SI{12(2.5)}{\gpkg}, \SI{11(2.3)}{\gpkg}, and \SI{18(3.7)}{\gpkg} of phosphorus, calcium, and potassium in nowadays' feeds, respectively, while \SI{14.9(0.02)}{\gpkg}, \SI{9.6(1.0)}{\gpkg}, and \SI{20.1(2.7)}{\gpkg} were reported before [@Tacon1983]. Magnesium inclusion rates are slightly more variable, ranging from \SIrange{1}{3}{\gpkg} with an average of \SI{2(0.8)}{\gpkg} (\SI{2.1(0.1)}{\gpkg} in the past) and a \gls{cv} of \SI{36}{\p}. 

In comparison with past values, the highest increase in their inclusion rates was found for the microminerals. 

The iron content of aquafeeds increased by `r (1-183.5/209)*100`\si{\p}, with an average of \SI{209(237.1)}{\mgpkg} in comparison with \SI{183.5(24.7)}{\mgpkg} in the 90ies. Iron is also the nutrient with the highest variability in aquafeeds, indicated by the \gls{cv} of \SI{114}{\p} and a range from \SIrange{40}{544}{\mgpkg}.

Nowadays, aquafeeds are richer in copper than in the past. The average inclusion rate increased by `r (1-15/20)*100`\si{\p}, from \SI{15(1.4)}{\mgpkg} to \SI{20(18.5)}{\mgpkg}. In the case of zinc, inclusion rates increased by `r (1-90/207)*100`\si{\p}, from \SI{90(7)}{\mgpkg} to \SI{207(126.6)}{\mgpkg}. The inclusion rates of both copper and zinc were again highly variable, with \gls{cv} of \SI{93}{\p} and \SI{61}{\p}, and ranges from \SIrange{5}{46}{\mgpkg} and \SIrange{90}{384}{\mgpkg}, respectively. 

```{=tex}
\begin{sidewaystable}
  \caption{Synthesis of the mineral composition of starter and grower feeds for freshwater fishes suitable for aquaponic systems. Data covers compositions of fingerling and grower trout feeds reported by Tacon (1983), a compilation of feed compositions for African catfish, Common carp, Pikeperch, Nile tilapia, and Trout, the composition of speciality feeds for aquaponic systems, and some experimental feeds used in aquaponics trials. N, P, K, Ca, Mg: \si{\gpkg}. Fe, Mn, Cu, Zn: \si{\mgpkg}.}
  \label{tab:feedcomp}
  \vspace{1em}
...
data_commercial_special <- data$feed_commercial[[1]] %>% 
  create_plot_data() %>% 
  filter(target_system %in% c("aquaponics")) %>% 
  select(c(producer:feed_category4, name:value)) %>% 
  drop_na(value) %>% 
  group_by(name, target_system) %>% 
  summarise(
    n = n(),
    min = min(value, na.rm = TRUE),
    q1 = quantile(value, na.rm = TRUE, 0.25),
...
## Aquaponics feeds
There are also some commercial feeds available that are marketed as specifically formulated for aquaponic systems (n = 3). Though, an in-depth characterisation of these feeds is lacking. 
...
Even though available data is limited, it can be seen that the phosphorus content of these aquafeeds is highly variable (\gls{cv} = \SI{76}{\p}). The average phosphorus content of \SI{22(17)}{\gpkg}, meanwhile, is not significantly different from the unspecific diets (t-test: p = 0.4). Information with regards to potassium and calcium inclusion rates were reported only for one feed. Here, potassium and calcium inclusion rates were `r (1-20/11)*100`\si{\p} higher and `r (1-15/18)*100`\si{\p} lower than the average of unspecific diets, respectively. Other data with regards to these feeds is not available to the best of the author's knowledge.
...
## Experimental feeds
In the recent past, experiments were conducted to closer examine the manipulability of substance excretion patterns by fish, for instance in relation to alterations in the raw material composition.
...
- several experimental feeds were developed for aquaponics experiments to study wether nutrient excretion patterns might differ between feed ingredients.
- overall nutrient composition comparable to commercial feeds
...
data_experimental_studies <- data$feed_experimental[[1]] %>% 
  create_plot_data() %>% 
  drop_na(value) %>% 
  group_by(name) %>% 
  summarise(
...
```{r}
bind_rows(list(commercial = data_commercial, experimental = data_experimental_studies), .id = "source") %>% 
  arrange(name) %>% 
  print()
```
...
In fish, the essentiality of several macrominerals (calcium, phosphorus, magnesium, sodium, potassium, chloride) and microminerals (iron, copper, manganese, zinc, iodine, selenium, cobalt, and chromium) was shown [@Lall2021]. These elements play a vital role in the development and maintenance of the skeletal system, processes such as signal transmission, osmoregulation, as reactive centers of metalloenzymes, and others. A comprehensive summary can be found elsewhere (see e.g., Lall, 2003). Out of these, calcium, phosphorus, magnesium, sodium, iron, copper, manganese, and zinc are relevant for plant growth [@Marschner]. 



 these substances are not 

This circumstance is due to the current regulations affecting the labeling. Currently, general mandatory and specific requirements with regards to the labelling of compound feeds are defined by Regulation (EC) No. 767/2009 of the European Parliament defines in the articles 15 and 17 and a detailed explanation with respect to the labelling of feed materials and compound feed for animals intended for human consumption can be found in Annex VI, stating that the levels of crude protein, crude fibre, crude oils and fats, crude ash, calcium, sodium and phosphorus need to be listed by the feed producer (EU). This 




In fish, the essentiality of several macrominerals (calcium, phosphorus, magnesium, sodium, potassium, chloride) and microminerals (iron, copper, manganese, zinc, iodine, selenium, cobalt, and chromium) was shown [@Lall2021]. These elements play a vital role in the development and maintenance of the skeletal system, processes such as signal transmission, osmoregulation, as reactive centers of metalloenzymes, and others. A comprehensive summary can be found elsewhere (see e.g., Lall, 2003).
While macrominerals are taken up in relatively large quantities by fish, only small amounts of others are required of the microminerals. However, exact nutritional requirements are unknown for many of the mentioned substances due to difficulties in their determination, as described elsewhere (see e.g., Lall, 2003). In addition, uptake from the water via gills and skin contributes significantly to the nutrient budget, enabling the organism to reach homeostasis. Phosphorus, magnesium, iron, zinc, and selenium are mainly taken up via the feed.
...
Water is added to the system either to compensate evaporation, spillage or leaning losses (e.g., the drum filter backwash) or to dilute accumulating substances such as nitrate [@Maucieri2018]. The source water can originate from wells, collected rain or the municipal tap.
