...
```{r}
selection <- data$water_tap[[1]] %>% 
  select(Reference_ID:GPS, matches("belowLimit")) %>% 
  pivot_longer(cols = NH4_belowLimit:TOC_belowLimit,
               names_to = "analyte",
               values_to = "limit") %>% 
  mutate(analyte = str_remove(analyte, "_belowLimit"))

data$water_tap[[1]] %>% 
  select(Reference_ID:GPS, matches("mgL")) %>% 
  pivot_longer(cols = NH4_mgL:TOC_mgL,
               names_to = "analyte",
               values_to = "value_mgL") %>% 
  mutate(analyte = str_remove(analyte, "_mgL"),
         value_ugL = value_mgL*1e3) %>% 
  left_join(selection) %>% 
  filter(limit == FALSE) %>% 
  rename(value = "value_ugL") %>% 
  group_by(analyte) %>% 
  summarise(
    n = n(),
    min = min(value, na.rm = TRUE),
...
While tap water is rich in the macronutrients calcium, sulfur, and magnesium, those substances are comparably little variable among sampling sites. An average calcium concentration of \SI{59.9(3.0)}{\mgL} 
sulfur concentration of 
The magnesium concentration in tap water was found to be on average \SI{12.0(8.0)}{\mgL}, with a total range of \SIrange{0.1}{4.0}{\mgL} in the dataset. 
