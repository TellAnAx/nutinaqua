...
data$feed_cleaned <- data$feed[[1]] %>% 
  ...
  write_rds(here("interm", "feed_stattest.rds")) %>% # write data for statistical analysis
  create_plot_data() %>% 
  drop_na(value) %>% 
...
Aquafeeds are considered the most important nutrient input into aquaculture systems. This is acknowledged by the availability of specific feeds for aquaculture in open (ponds or cages) and closed (\gls{ras}) systems to reduce the nutrient load of effluents or improve faeces stability and thus removability by mechanical filtration. 
